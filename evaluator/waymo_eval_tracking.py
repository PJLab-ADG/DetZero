...
class WaymoTrackingMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False, fake_gt_infos=True):
        ...
        frame_id, sequence_id, object_id, boxes3d, obj_type, score, overlap_nlz, difficulty, speed = [], [], [], [], [], [], [], [], []
        sequence_id_dict = {}
        object_id_dict = {}
        for frame_index, info in enumerate(infos):
            if is_gt:
                annos = info['annos']
                box_mask = np.array([n in class_names for n in annos['name']], dtype=np.bool_)
                if 'num_points_in_gt' in annos:
                    zero_difficulty_mask = annos['difficulty'] == 0
                    annos['difficulty'][(annos['num_points_in_gt'] > 5) & zero_difficulty_mask] = 1
                    annos['difficulty'][(annos['num_points_in_gt'] <= 5) & zero_difficulty_mask] = 2
                    nonzero_mask = annos['num_points_in_gt'] > 0
                ...
                box_name = annos['name'][box_mask]

                difficulty.append(annos['difficulty'][box_mask])
                score.append(np.ones(num_boxes))
                if fake_gt_infos:
                    annos['gt_boxes_lidar'] = boxes3d_kitti_fakelidar_to_lidar(annos['gt_boxes_lidar'])

                boxes3d.append(annos['gt_boxes_lidar'][box_mask][:, :7])
                speed.append(annos['gt_boxes_lidar'][box_mask][:, 7:])
                for obj_id in annos['obj_ids'][box_mask]:
                    if not obj_id in object_id_dict:
                        object_id_dict[obj_id] = len(object_id_dict)
                    object_id.append(object_id_dict[obj_id])
            else:
                num_boxes = len(info['boxes_lidar'])
                difficulty.append([0] * num_boxes)
                score.append(info['score'])
                boxes3d.append(np.array(info['boxes_lidar'][:, :7]))
                speed.append(np.array(info['boxes_lidar'][:, 7:]))
                box_name = info['name']
                
                sequence_name = info['sequence_name']
                for obj_id in info['obj_ids']:
                    new_obj_id = sequence_name + '-' + str(obj_id)
                    if not new_obj_id in object_id_dict:
                        object_id_dict[new_obj_id] = len(object_id_dict)
                    object_id.append(object_id_dict[new_obj_id])

            if not info['sequence_name'] in sequence_id_dict:
                sequence_id_dict[info['sequence_name']] = len(sequence_id_dict)

            obj_type += [self.WAYMO_CLASSES.index(name) for i, name in enumerate(box_name)]
            frame_id.append(np.array([frame_index] * num_boxes))
            sequence_id.append(np.array([sequence_id_dict[info['sequence_name']]]*num_boxes))
        ...
        return frame_id, sequence_id, object_id, boxes3d, obj_type, score, overlap_nlz, difficulty, speed
    ...
    def build_graph(self, graph, config_type):
        with graph.as_default():
            ...
            self._pd_sequence_id = tf.compat.v1.placeholder(dtype=tf.string)
            self._pd_object_id = tf.compat.v1.placeholder(dtype=tf.int64)
            ...
            self._gt_sequence_id = tf.compat.v1.placeholder(dtype=tf.string)
            self._gt_object_id = tf.compat.v1.placeholder(dtype=tf.int64)
            ...
            self._gt_speed = tf.compat.v1.placeholder(dtype=tf.float32)
            metrics = tracking_metrics.get_tracking_metric_ops(
                ...
                prediction_sequence_id=self._pd_sequence_id,
                prediction_object_id=self._pd_object_id,
                ...
                ground_truth_sequence_id=self._gt_sequence_id,
                ground_truth_object_id=self._gt_object_id,
                ground_truth_difficulty=self._gt_difficulty,
                ground_truth_speed=self._gt_speed,
            )
            return metrics
    ...
    def run_eval_ops(
        ...
        sess.run(
            ...
            feed_dict={
                ...
                self._pd_sequence_id: prediction_sequence_id,
                self._pd_object_id: prediction_object_id,
                ...
                self._gt_sequence_id: ground_truth_sequence_id,
                self._gt_object_id: ground_truth_object_id,
                self._gt_difficulty: ground_truth_difficulty,
                self._gt_speed: ground_truth_speed,
            },
        )
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, config_type, distance_thresh=100, fake_gt_infos=True):
        ...
        pd_frameid, pd_sequenceid, pd_objectid, pd_boxes3d, pd_type, pd_score, pd_overlap_nlz, _ , pd_speed= self.generate_waymo_type_results(
            prediction_infos, class_name, is_gt=False, fake_gt_infos=fake_gt_infos
        )
        gt_frameid, gt_sequenceid, gt_objectid, gt_boxes3d, gt_type, gt_score, gt_overlap_nlz, gt_difficulty, gt_speed = self.generate_waymo_type_results(
            gt_infos, class_name, is_gt=True, fake_gt_infos=fake_gt_infos
        )

        pd_boxes3d, pd_frameid, pd_sequenceid, pd_objectid, pd_type, pd_score, pd_overlap_nlz, pd_speed = self.mask_by_distance(
            distance_thresh, pd_boxes3d, pd_frameid, pd_sequenceid, pd_objectid, pd_type, pd_score, pd_overlap_nlz, pd_speed
        )
        gt_boxes3d, gt_frameid, gt_sequenceid, gt_objectid, gt_type, gt_score, gt_difficulty, gt_speed = self.mask_by_distance(
            distance_thresh, gt_boxes3d, gt_frameid, gt_sequenceid, gt_objectid, gt_type, gt_score, gt_difficulty, gt_speed
        )
        ...
        with self.test_session(graph=graph) as sess:
            ...
            self.run_eval_ops(
                sess, graph, metrics, pd_frameid, pd_sequenceid, pd_objectid, pd_boxes3d, pd_type, pd_score, pd_overlap_nlz,
                gt_frameid, gt_sequenceid, gt_objectid, gt_boxes3d, gt_type, gt_difficulty, gt_speed
            )
            with tf.compat.v1.variable_scope('tracking_metrics', reuse=True):
                motas = self.eval_value_ops(sess, graph, metrics)
...
def main():
    ...
    eval = WaymoTrackingMetricsEstimator()
