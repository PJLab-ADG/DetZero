...
class CenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names,
        ...
        self.TTA = tta
        self.iou_weight = self.model_cfg.get("IOU_WEIGHT", 0)
    ...
    def assign_target_of_single_head(self, num_classes, gt_boxes, feature_map_size, feature_map_stride,
    ...
    def get_loss(self, tb_dict=None):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            tb_dict['center_z_loss_head_%d' % idx] = (temp_loss[2]).item()
            tb_dict['dim_loss_head_%d' % idx] = (temp_loss[3:6].sum()).item()
            tb_dict['rot_loss_head_%d' % idx] = (temp_loss[6:8].sum()).item()
            if 'vel' in self.separate_head_cfg.HEAD_ORDER:
                tb_dict['vel_loss_head_%d' % idx] = (temp_loss[8:10].sum()).item()

            temp_loss = temp_loss.sum()
            loc_loss = temp_loss * self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['loc_weight']
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            if self.predict_boxes_when_training:
                ...
                for batch_idx in range(batch_size):
                    ...
                    # N x C
                    feature_map = centernet_utils.bilinear_interpolate_torch(
                        bev_features[batch_idx], xs, ys)
