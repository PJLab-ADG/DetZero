...
from detzero_utils.ops.iou3d_nms.iou3d_nms_utils import boxes_iou3d_gpu, boxes_iou_bev


def prefilter_boxes(boxes, scores, labels, weights, thresh, obj_ids=None):
    ...
    for i in range(len(boxes)):
        ...
        if obj_ids is not None and len(boxes[i]) != len(obj_ids[i]):
            raise ValueError("Length of boxes not equal to length of obj_ids.")
        ...
        for j in range(len(boxes[i])):
            ...
            if obj_ids is not None:
                obj_id = int(obj_ids[i][j][0])
                new_box = [int(label), float(score)*weights[i], obj_id, x, y, z, dx, dy, dz, yaw]
            else:
...
def get_weighted_box(boxes, conf_type='avg'):
    ...
    """
    box_dim = len(boxes[0])
    weighted_box = np.zeros(box_dim, dtype=np.float32)
    conf = 0
    conf_list = []
    for box in boxes:
        weighted_box[-7:] += (box[1] * box[-7:])
        conf += box[1]
    ...
    if box_dim > 9:
        obj_id_list = np.array([box[2] for box in boxes])
        sort_idx = np.argsort(np.array(conf_list))[::-1]
        obj_id_list = obj_id_list[sort_idx]
        obj_id_list = obj_id_list[obj_id_list >= 0]
        if len(obj_id_list):
            weighted_box[2] = obj_id_list[0]
        else:
            weighted_box[2] = -1
...
def find_matching_box(boxes_list, new_box, iou_thresh, iou_type):
    ...
    boxes_gpu = copy.deepcopy(torch.from_numpy(boxes_list[:, -7:]).float().cuda())
    new_box = torch.from_numpy(new_box[-7:]).unsqueeze(0).float().cuda()
    if iou_type == '3d':
        ious = boxes_iou3d_gpu(new_box, boxes_gpu)
    elif iou_type == 'bev':
        ious = boxes_iou_bev(new_box, boxes_gpu)
...
def weighted_boxes_fusion_3d(boxes_list, scores_list, labels_list,
    ...
    boxes = overall_boxes[:, -7:]
    scores = overall_boxes[:, 1]
    labels = overall_boxes[:, 0].astype(int)
    
    return boxes, scores, labels

def weighted_tracking_boxes_fusion_3d(boxes_list, scores_list, labels_list, obj_ids_list,
                                      weights=None, iou_thr=None, skip_box_thr=None,
                                      conf_type='avg', iou_type='3d',
                                      allows_overflow=False):
    '''
    ...
    if conf_type not in ['avg', 'max']:
        print('Error. Unknown conf_type: {}. Must be "avg" or "max". Use "avg"'.format(conf_type))
        conf_type = 'avg'
    filtered_boxes = prefilter_boxes(boxes_list, scores_list, labels_list, weights, skip_box_thr, obj_ids_list)
    if len(filtered_boxes) == 0:
        return np.zeros((0, 7)), np.zeros((0,)), np.zeros((0,)), np.zeros((0,))
    ...
    scores = overall_boxes[:, 1]
    labels = overall_boxes[:, 0].astype(int)
    obj_ids = overall_boxes[:, 2].astype(int)
