...
def _topk(scores, batch_iou=None, K=40, **kwargs):
    ...
    if kwargs.get('id_map', None) is not None:
        id_maps = kwargs['id_map'].view(batch, num_class, -1)
        id_maps = id_maps.gather(2, topk_inds)
    ...
    if kwargs.get('id_map', None) is not None:
        id_maps = _gather_feat(id_maps.view(batch, -1, 1), topk_ind).view(batch, K)
        return topk_score, topk_inds, topk_classes, topk_ys, topk_xs, id_maps
    else:
