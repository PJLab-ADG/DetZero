...
__device__ inline int check_pt_in_box2d(const float *pt, const float *box3d, float &local_x, float &local_y){
    // param pt: (x, y)
    ...
    float dx = box3d[2], dy = box3d[3], rz = box3d[4];
...
__global__ void points_in_boxes_v2_kernel(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z] in LiDAR coordinate
    // params boxes_idx_of_points: (B, N, npoints), default -1
    ...
    boxes += bs_idx * boxes_num * 7;
    pts += bs_idx * pts_num * 3 + pt_idx * 3;
    box_idx_of_points += bs_idx * boxes_num * pts_num + pt_idx;
...
__global__ void points_in_boxes_num_kernel(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *points_in_box_num){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z] in LiDAR coordinate
    // params boxes_idx_of_points: (B, npoints), default -1
    ...
    boxes += bs_idx * boxes_num * 7;
    pts += bs_idx * pts_num * 3 + pt_idx * 3;
    points_in_box_num += bs_idx * boxes_num;
    ...
    for (int k = 0; k < boxes_num; k++){
        cur_in_flag = check_pt_in_box3d(pts, boxes + k * 7, local_x, local_y);
        if (cur_in_flag){
            atomicAdd(points_in_box_num+k, 1);
        }
    }
}


__global__ void points_in_boxes_2dkernel(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, dx, dy, heading] (x, y) is the box center
    ...
    int cur_in_flag = 0;
    for (int k = 0; k < boxes_num; k++){
        cur_in_flag = check_pt_in_box2d(pts, boxes + k * 5, local_x, local_y);
        if (cur_in_flag){
            box_idx_of_points[0] = k;
            break;
        }
...
__global__ void points_in_boxes_2dkernel_v2(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *pts_indices){
    // params boxes: (B, N, 7) [x, y, dx, dy, heading] (x, y) is the box center
    // params pts: (B, npoints, 2) [x, y] in LiDAR coordinate
    // params boxes_idx_of_points: (B, npoints), default -1
    ...
    boxes += bs_idx * boxes_num * 5;
    pts += bs_idx * pts_num * 2 + pt_idx * 2;
    pts_indices += bs_idx * pts_num * boxes_num + pt_idx * boxes_num;
...
void points_in_boxes_v2_launcher(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params boxes_idx_of_points: (B, npoints), default -1
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_boxes_v2_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
...
void points_in_boxes_num_launcher(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *points_in_box_num){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params N: (B, N), default 0
    cudaError_t err;

    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);

    points_in_boxes_num_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, points_in_box_num);
...
void points_in_boxes_2dlauncher(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 5) [x, y, dx, dy heading] (x, y, 5) is the box center
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_boxes_2dkernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
...
void points_in_boxes_2dlauncher_v2(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *pts_indices){
    // params boxes: (B, N, 5) [x, y, dx, dy heading] (x, y, 5) is the box center
    // params pts: (B, npoints, 2) [x, y]
    // params pts_indices: (B, npoints, N), default 0
    cudaError_t err;

    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_boxes_2dkernel_v2<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, pts_indices);
