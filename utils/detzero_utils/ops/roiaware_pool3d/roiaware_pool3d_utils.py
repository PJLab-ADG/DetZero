...
def points_in_boxes_gpu_v2(points, boxes):
    ...
    :param points: (B, M, 3)
    :param boxes: (B, T, 7), num_valid_boxes <= T
    :return box_idxs_of_pts: (B, T, M), default background = 0
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 7 and points.shape[2] == 3
    batch_size, num_points, _ = points.shape

    box_idxs_of_pts = points.new_zeros((batch_size, boxes.shape[1], num_points), dtype=torch.int).fill_(0)
    roiaware_pool3d_cuda.points_in_boxes_gpu_v2(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)
...
def points_in_boxes_num_gpu(points, boxes):
    ...
    num_points_in_box = points.new_zeros((batch_size, box_num), dtype=torch.int).fill_(0)
    roiaware_pool3d_cuda.points_in_boxes_num_gpu(boxes.contiguous(), points.contiguous(), num_points_in_box)
...
def points_in_boxes_2d_gpu(points, boxes):
    ...
    box_idxs_of_pts = points.new_zeros((batch_size, num_points), dtype=torch.int).fill_(-1)
    roiaware_pool3d_cuda.points_in_boxes_2d_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)
...
def points_in_boxes_2d_gpu_v2(points, boxes):
    ...
    point_indices = points.new_zeros((batch_size, num_points, boxes.shape[1]), dtype=torch.int)
    roiaware_pool3d_cuda.points_in_boxes_2d_gpu_v2(boxes.contiguous(), points.contiguous(), point_indices)
